    ...
    var GetDisscussInfo = function(){
        ...
        YZQ_UTILS.AJAXRequest(
            ...
            function(result){
                ...
                infodata.release_time=YZQ_COMMON.formateTime(infodata.release_time,_type);
                ...
                if (infodata.imgs.length>0){
                    for (i=0; i<infodata.imgs.length&&i<9; i++){
                        if(infodata.imgs[i].img_url ===""||infodata.imgs[i].img_url ===null){
                            ...
                            if (infodata.imgs[i].img_url.indexOf("hzyzq") > 0) {
                                infodata.imgs[i].img_url += "/thumbnail?max_age=19830212&d=20151230193947";
                            }else {
                                infodata.imgs[i].img_url += "?imageView2/1/w/200/format/yjpg/q/70";
                            }
                        }
                    }
...
//        var user_id =1;
    ...
    $('body').on("click",'#js_toprprise',function(){
        if($('#js_toprprise').attr('class') == "click-like"){
            ...
            if ($(this).parent().find(".zan-list").length == 0) {
                ...
                str += '<i class="head-img" data-pointid="20007221" style="background-image:url(' + userheadimg + ')"></i>';
    ...
    $('body').on('click','#js_replay_content',function(event){
        ...
        if(userid===""||userid===null){
            //跳转登录
            document.getElementById("js_replay_content").readOnly=true;
            HZ_APP_JSSDK.userloginjump();

        }else{
            document.getElementById("js_replay_content").readOnly=false;
            //对帖子回复
    ...
    $('body').on("click",'.ping',function(){
        ...
        try{
            ...
            if(userid===""||userid===null){
                ...
                document.getElementById("js_replay_content").focus();
                //对帖子中的评论回复
                $('body').find('#js_cont_sendto_victior').unbind("click").click(function(){
                    if($('#js_replay_content').val() == ""){
                        YZQ_COMMON.tips("请先输入评论再发送");
            ...
            function(result){
                ...
                if(result.status == 200){
                    ...
                    if(replyflag == 1){
                        ...
                        str += '<span class="cont-sp" data-id="'+data.id+'" data-pointid="20007211"><i class="jubao-v1"></i><!--yijing-jubao-v1--></span>';
                        str += '</div>';
                        str += '</li>';
                    ...
                    $('#js_cont_sendto_victior').hide();
                    $('#js_cont_send').show();
                    $('#js_replay_content').attr('placeholder','回复楼主：');
                    $('body').find('#js_cont_send').unbind("click").click(function(){
                        if($('#js_replay_content').val() == ""){
                            YZQ_COMMON.tips("请先输入评论再发送");
